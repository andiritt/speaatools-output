...
def process_json(json_path):
    ...
    laptime_std = np.std([car['raceLaptime'] for car in data])
    ...
    with open(md_path, "w") as mdfile:
        ...
        mdfile.write(f"- Laptime Std Dev: {laptime_std:.2f} seconds\n")
        mdfile.write(f"- Filtered Average Topspeed: {avg_topspeed_str}\n\n")
